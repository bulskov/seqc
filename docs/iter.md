# iter

Lazy iterator pipeline. Every data source exposes an `iter_t`; adaptors transform
one `iter_t` into another; terminals consume the iterator and produce a result.
Nothing allocates or runs until a terminal is called.

**Header:** `include/seqc/iter.h`  
**See also:** [`arena`](arena.md) · [`slice`](slice.md) · [`vec`](vec.md)

---

## Type

### `iter_t`

```c
struct iter_t {
  bool (*next)(iter_t *it, void *out); /* write elem_size bytes; return true/false */
  void (*drop)(iter_t *it);            /* free internal state; NULL is valid       */
  void      *state;
  size_t     elem_size;
  allocator_t  allocator;
};
```

Adaptors that need to allocate state (window, chunks, flat_map, …) use
`it.allocator`. The arena that owns the state is freed when `iter_drop()` is
called, or implicitly by the terminal.

### `iter_drop` (inline)

```c
static inline void iter_drop(iter_t *it);
```

Release iterator resources without consuming elements. Call this when you want
to abandon an iterator early (e.g. after `iter_find`).

---

## Function-pointer types

| Type          | Signature                                                | Used by                                                                                  |
| ------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `pred_fn`     | `bool fn(const void *elem, void *ctx)`                   | `iter_filter`, `iter_find`, `iter_any`, `iter_all`, `iter_take_while`, `iter_skip_while` |
| `map_fn`      | `void fn(const void *in, void *out, void *ctx)`          | `iter_map`                                                                               |
| `combine_fn`  | `void fn(void *acc, const void *elem, void *ctx)`        | `iter_reduce`                                                                            |
| `visitor_fn`  | `void fn(const void *elem, void *ctx)`                   | `iter_foreach`                                                                           |
| `compare_fn`  | `int fn(const void *a, const void *b)`                   | `iter_sort`, `iter_min`, `iter_max`, sorted containers                                   |
| `flat_map_fn` | `void fn(const void *elem, iter_t *out, void *ctx)`        | `iter_flat_map`                                                                          |  | `hash_fn` | `size_t fn(const void *key, size_t key_size)` | `hashmap_create`, `set_create` |
| `eq_fn`       | `bool fn(const void *a, const void *b, size_t key_size)` | `hashmap_create`, `set_create`                                                           |

---

## seqc_status_t

All mutating operations that can fail return a `seqc_status_t` (defined in
`iter/iter.h`).

```c
typedef enum {
    SEQC_OK        = 0,  /* success — treat as false in error checks  */
    SEQC_NOT_FOUND = 1,  /* element absent or collection empty        */
    SEQC_DUPLICATE = 2,  /* element already present (sets, trees)     */
    SEQC_OOM       = 3,  /* allocation failed                         */
    SEQC_INVALID   = 4,  /* invalid argument (e.g. NULL out pointer)  */
} seqc_status_t;
```

The zero value `SEQC_OK` is falsy so you can write either style:

```c
if (vec_push(v, &x))       { /* error */ }   /* truthy = any failure */
if (vec_push(v, &x) != SEQC_OK) { /* error */ }   /* explicit */
```

Predicate functions (`_contains`, `_is_empty`, `_is_healthy`, `string_equals`,
…) keep `bool` return types and are unaffected.

---

## Sources

Sources create an `iter_t` from an existing data structure.

### `iter_from_slice`

```c
iter_t iter_from_slice(slice_t s, allocator_t allocator);
```

Iterate over a [`slice_t`](slice.md) in order. `allocator` is inherited by
downstream adaptors.

```c
int nums[] = {1, 2, 3};
slice_t s = {nums, 3, sizeof(int)};
iter_t it = iter_from_slice(s, arena_allocator(a));
```

### `iter_from_slice_rev`

```c
iter_t iter_from_slice_rev(slice_t s, allocator_t allocator);
```

Same as `iter_from_slice` but yields elements in reverse order.

---

### `iter_generate`

```c
typedef bool (*generate_fn)(void *out, void *ctx);
iter_t iter_generate(generate_fn fn, void *ctx, size_t elem_size,
                   allocator_t allocator);
```

Create an iterator that calls `fn(out, ctx)` on each `next` call. `fn` writes
`elem_size` bytes into `out` and returns `true` to continue or `false` to
signal exhaustion. Useful for stateful generators or wrapping external sources.

```c
/* Fibonacci generator */
typedef struct { long long a, b; } fib_state_t;

static bool fib_next(void *out, void *ctx) {
    fib_state_t *s = ctx;
    *(long long *)out = s->a;
    long long tmp = s->a + s->b;
    s->a = s->b;
    s->b = tmp;
    return true;  /* infinite — pair with iter_take */
}

fib_state_t state = {0, 1};
slice_t first10 = iter_collect(
    iter_take(iter_generate(fib_next, &state, sizeof(long long),
                            arena_allocator(a)), 10),
    arena_allocator(a));
```

---

### `iter_range`

```c
iter_t iter_range(long long start, long long end, long long step,
                allocator_t allocator);
```

Yield `long long` integers from `start` (inclusive) to `end` (exclusive) with
the given `step`. If `step` is zero an empty iterator is returned. Yields
nothing if the range is already exhausted (e.g. `start >= end` with positive
step).

```c
/* 0, 1, 2, 3, 4 */
iter_t it = iter_range(0, 5, 1, arena_allocator(a));

/* 10, 8, 6, 4, 2 */
iter_t it = iter_range(10, 0, -2, arena_allocator(a));
```

Other sources live on their respective modules:

| Source                                                  | Module                |
| ------------------------------------------------------- | --------------------- |
| `vec_iter` / `vec_iter_rev`                             | [vec](vec.md)         |
| `stack_iter`                                            | [stack](stack.md)     |
| `queue_iter`                                            | [queue](queue.md)     |
| `list_iter`                                             | [list](list.md)       |
| `dlist_iter` / `dlist_iter_rev`                         | [dlist](dlist.md)     |
| `set_iter`                                              | [set](set.md)         |
| `hashmap_iter` / `hashmap_iter_rev`                     | [hashmap](hashmap.md) |
| `string_chars` / `string_chars_rev` / `string_split_substr` / `string_split_any` | [string](string.md)   |
| `bstree_iter` / `bstree_iter_rev` / `bstree_iter_range` | [bstree](bstree.md)   |
| `avl_iter` / `avl_iter_rev` / `avl_iter_range`          | [avl](avl.md)         |
| `omap_iter` / `omap_iter_rev` / `omap_iter_range`       | [omap](omap.md)       |

---

## Adaptors

Adaptors take ownership of their source `iter_t` and return a new one. They are
lazy — no work happens until `next()` is called.

### `iter_filter`

```c
iter_t iter_filter(iter_t source, pred_fn pred, void *ctx);
```

Yield only elements for which `pred(elem, ctx)` returns non-zero.

```c
static int is_positive(const void *e, void *ctx) {
    return *(const int *)e > 0;
}

iter_t it = iter_filter(vec_iter(&v), is_positive, NULL);
```

---

### `iter_map`

```c
iter_t iter_map(iter_t source, map_fn map, void *ctx, size_t out_elem_size);
```

Transform each element. The output type may differ from the input type;
`out_elem_size` must match the size written by `map`.

```c
static void to_double(const void *in, void *out, void *ctx) {
    *(double *)out = (double)*(const int *)in;
}

iter_t it = iter_map(vec_iter(&v), to_double, NULL, sizeof(double));
```

---

### `iter_take`

```c
iter_t iter_take(iter_t source, size_t n);
```

Yield at most `n` elements then stop.

---

### `iter_skip`

```c
iter_t iter_skip(iter_t source, size_t n);
```

Discard the first `n` elements then yield the rest.

---

### `iter_take_while`

```c
iter_t iter_take_while(iter_t source, pred_fn pred, void *ctx);
```

Yield elements from `source` as long as `pred` returns `true`. Stops
permanently at the first element that fails — that element is not yielded.

```c
static bool is_positive(const void *elem, void *ctx) {
    return *(const int *)elem > 0;
}

// {3, 7, -1, 5} → yields 3, 7  (stops at -1)
iter_t it = iter_take_while(vec_iter(&v), is_positive, NULL);
```

---

### `iter_skip_while`

```c
iter_t iter_skip_while(iter_t source, pred_fn pred, void *ctx);
```

Skip elements from `source` as long as `pred` returns `true`. Once the first
element fails the predicate, that element and all subsequent elements are
yielded unconditionally (even if they would satisfy the predicate).

```c
static bool is_positive(const void *elem, void *ctx) {
    return *(const int *)elem > 0;
}

// {3, 7, -1, 5} → yields -1, 5  (skips 3 and 7)
iter_t it = iter_skip_while(vec_iter(&v), is_positive, NULL);
```

---

### `iter_chain`

```c
iter_t iter_chain(iter_t a, iter_t b);
```

Yield all elements of `a`, then all elements of `b`. Both iterators must have
the same `elem_size`.

```c
iter_t combined = iter_chain(list_iter(&left), list_iter(&right));
```

---

### `iter_zip`

```c
iter_t iter_zip(iter_t a, iter_t b);
```

Yield interleaved pairs `[a_elem | b_elem]` as a flat buffer. Stops when either
source is exhausted. The result `elem_size` equals `a.elem_size + b.elem_size`.

```c
// zip int keys with double values
iter_t pairs = iter_zip(keys_iter, vals_iter);
// each element is {int, double} — cast accordingly
```

---

### `iter_enumerate`

```c
iter_t iter_enumerate(iter_t source);
```

Pair each element with its 0-based index. Yields `enum_entry_t`:

```c
typedef struct {
    size_t index;
    void  *elem;  /* pointer into internal buffer — valid until next call */
} enum_entry_t;
```

```c
iter_t it = iter_enumerate(list_iter(&l));
enum_entry_t e;
while (it.next(&it, &e))
    printf("[%zu] %d\n", e.index, *(int *)e.elem);
iter_drop(&it);
```

---

## Shared entry types

### `map_entry_t`

```c
typedef struct {
    void *key;
    void *value;
} map_entry_t;
```

Pointer-pair yielded by [`hashmap_iter`](hashmap.md#hashmap_iter) and
[`omap_iter`](omap.md#omap_iter). Both pointers point directly into live
bucket / node storage. Do not modify the map while iterating.

`hashmap.h` and `omap.h` each export a typedef alias
(`hashmap_entry_t` / `omap_entry_t`) for callers that prefer the module-prefixed
name.

---

### `iter_window`

```c
iter_t iter_window(iter_t source, size_t n);
```

Yield overlapping windows of size `n` as a [`slice_t`](slice.md) pointing into an
internal buffer. **Copy the slice if you need to keep it past the next call.**
Yields nothing if the source has fewer than `n` elements.

```c
// sliding 3-element window over [1,2,3,4,5]
iter_t it = iter_window(vec_iter(&v), 3);
slice_t win;
while (it.next(&it, &win))
    printf("%d %d %d\n", *(int *)slice_get(win,0),
                         *(int *)slice_get(win,1),
                         *(int *)slice_get(win,2));
iter_drop(&it);
```

---

### `iter_chunks`

```c
iter_t iter_chunks(iter_t source, size_t n);
```

Yield non-overlapping chunks of exactly `n` elements as a [`slice_t`](slice.md).
The last chunk may be smaller than `n`. **Copy the slice if you need to keep it
past the next call.**

---

### `iter_flat_map`

```c
typedef void (*flat_map_fn)(const void *elem, iter_t *out, void *ctx);
iter_t iter_flat_map(iter_t source, flat_map_fn fn, void *ctx,
                   size_t out_elem_size);
```

For each element in `source`, call `fn(elem, &sub, ctx)` which must populate
`*sub` with a sub-iterator. All sub-iterators are drained in order.
`out_elem_size` must match the `elem_size` of every sub-iterator produced.

```c
static void expand(const void *elem, iter_t *out, void *ctx) {
    // produce a vec_t of ints from each input int, then iterate it
    int n = *(const int *)elem;
    vec_t *v = ctx;
    vec_push(v, &n);
    vec_push(v, &n);
    *out = vec_iter(v);
}
```

---

### `iter_peekable`

```c
iter_t iter_peekable(iter_t source);
bool iter_peek(iter_t *it, void *out);  /* inline helper */
```

Wraps `source` so that `iter_peek` can inspect the next element without
consuming it. The peeked element is buffered internally and returned again by
the next call to `next`. Calling `iter_peek` on a non-peekable iterator returns
`false`.

```c
iter_t it = iter_peekable(vec_iter(v));
int ahead;
if (iter_peek(&it, &ahead) && ahead == SENTINEL) {
    /* handle sentinel without consuming it */
}
```

---

### `iter_dedup`

```c
iter_t iter_dedup(iter_t source, compare_fn cmp);
```

Skip consecutive elements that compare equal (i.e. `cmp` returns `0`).
Non-adjacent duplicates are kept. Pair with `iter_sort` to yield only unique
values.

```c
/* collect unique sorted values */
slice_t unique = iter_collect(
    iter_dedup(
        iter_from_slice(iter_sort(iter_from_slice(s, al), int_cmp, al), al),
        int_cmp),
    al);
```

---

## Terminals

Terminals consume the iterator and free its resources.

### `iter_collect` {#iter_collect}

```c
slice_t iter_collect(iter_t it, allocator_t allocator);
```

Materialise all elements into an arena-owned [`slice_t`](slice.md). `allocator`
determines which arena owns the returned memory. Pass the same arena you used
for the iterator chain, or any other arena whose lifetime exceeds the slice.

```c
Arena *a = arena_create(4096);
slice_t s = iter_collect(
    iter_map(vec_iter(&v), double_fn, NULL, sizeof(double)),
    arena_allocator(a));
```

---

### `iter_count`

```c
size_t iter_count(iter_t it);
```

Consume the iterator and return the number of elements yielded.

---

### `iter_foreach`

```c
void iter_foreach(iter_t it, visitor_fn visit, void *ctx);
```

Call `visit(elem, ctx)` for every element. Useful when side-effects are all
you need.

```c
static void print_int(const void *e, void *ctx) {
    printf("%d\n", *(const int *)e);
}
iter_foreach(vec_iter(&v), print_int, NULL);
```

---

### `iter_reduce`

```c
void iter_reduce(iter_t it, void *acc, combine_fn combine, void *ctx);
```

Fold all elements into `acc`. `combine(acc, elem, ctx)` is called for each
element; the caller initialises `acc` before calling.

```c
static void sum(void *acc, const void *elem, void *ctx) {
    *(int *)acc += *(const int *)elem;
}

int total = 0;
iter_reduce(vec_iter(&v), &total, sum, NULL);
```

---

### `iter_sort` {#iter_sort}

```c
slice_t iter_sort(iter_t it, compare_fn cmp, allocator_t allocator);
```

Collect all elements into `allocator` then sort in place using `cmp`. Returns
the sorted [`slice_t`](slice.md).

```c
static int int_cmp(const void *a, const void *b) {
    return *(const int *)a - *(const int *)b;
}
Arena *a = arena_create(4096);
slice_t sorted = iter_sort(vec_iter(&v), int_cmp, arena_allocator(a));
```

---

### `iter_find`

```c
bool iter_find(iter_t it, pred_fn pred, void *ctx, void *out);
```

Return `true` and write the first matching element to `*out` (may be `NULL`) if
found; return `false` otherwise. The iterator is dropped after use.

---

### `iter_any`

```c
bool iter_any(iter_t it, pred_fn pred, void *ctx);
```

Return `true` if any element satisfies `pred`. Short-circuits on first match.

---

### `iter_all`

```c
bool iter_all(iter_t it, pred_fn pred, void *ctx);
```

Return `true` if every element satisfies `pred` (vacuously true for empty
iterators). Short-circuits on first failure.

---

### `iter_min`

```c
bool iter_min(iter_t it, compare_fn cmp, void *out);
```

Write the minimum element to `*out` (`out` may be `NULL`). Returns `true` on
success, `false` if the iterator was empty.

---

### `iter_max`

```c
bool iter_max(iter_t it, compare_fn cmp, void *out);
```

Write the maximum element to `*out` (`out` may be `NULL`). Returns `true` on
success, `false` if the iterator was empty.

---

## Full pipeline example

```c
#include "seqc/arena.h"
#include "seqc/vec.h"
#include "seqc/iter.h"

static bool is_even(const void *e, void *ctx)  { return *(const int *)e % 2 == 0; }
static void triple(const void *in, void *out, void *ctx) {
    *(int *)out = *(const int *)in * 3;
}
static int int_cmp(const void *a, const void *b) {
    return *(const int *)a - *(const int *)b;
}

int main(void) {
    Arena *a = arena_create(4096);
    vec_t    v = vec_create(sizeof(int), arena_allocator(a));

    for (int i = 10; i >= 1; i--) vec_push(&v, &i);

    // keep evens, triple, sort ascending
    slice_t result = iter_sort(
                       iter_map(
                           iter_filter(vec_iter(&v), is_even, NULL),
                           triple, NULL, sizeof(int)),
                       int_cmp, arena_allocator(a));
    // result = {6, 12, 18, 24, 30}

    arena_free(a);
}
```
